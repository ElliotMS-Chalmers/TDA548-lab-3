...
class Game:
    ...
    def newRound(self):
        self.setCurrentWind(random.randrange(-10, 10))
...
class Player:
    ...
    def fire(self, angle, velocity):
        ...
        angle = 180 - angle if self.isReversed else angle
        self.lastVelocity = velocity
        return Projectile(angle, velocity, self.game.getCurrentWind(), self.xPos, self.game.getCannonSize() / 2, -110, 110)

    """ Gives the x-distance from this players cannon to a projectile. If the cannon and the projectile touch (assuming the projectile is on the ground and factoring in both cannon and projectile size) this method should return 0"""
    def projectileDistance(self, proj):
        distance_center = proj.getX() - self.xPos
        cannon_radius = self.game.getCannonSize() / 2
        ball_radius = self.game.getBallSize()
        
        if abs(distance_center) < cannon_radius + ball_radius: # Hit
            distance = 0                                                
        elif distance_center < 0: # Left             
            distance = distance_center + cannon_radius + ball_radius    
        elif distance_center > 0: # Right
            distance = distance_center - cannon_radius - ball_radius    
